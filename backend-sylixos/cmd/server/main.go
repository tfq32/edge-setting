...
func main() {
	...
	// ── 定时任务 ──────────────────────────────────────────
	go runMetricsTicker(coll, db, hub)
	go runCleanupTicker(db)
...
func runMetricsTicker(coll *collector.Collector, db *store.DB, hub *ws.Hub) {
...
func runCleanupTicker(db *store.DB) {
	ticker := time.NewTicker(1 * time.Hour)
	defer ticker.Stop()
	for range ticker.C {
		if err := db.Cleanup(config.Global.Data.MetricsRetain); err != nil {
