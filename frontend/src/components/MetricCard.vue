<template>
  <div class="metric-card" :class="statusClass" @click="$emit('click')">
    <div class="mc-header">
      ...
      <svg class="mc-ring" viewBox="0 0 36 36" width="44" height="44">
    ...
    <div v-if="status !== 'normal'" class="mc-badge" :class="`mc-badge--${status}`">
...
.metric-card {
  ...
  border: 1px solid var(--border-subtle); box-shadow: 0 2px 10px var(--shadow);
  padding: 14px; cursor: pointer; transition: border-color .2s; position: relative; overflow: hidden;
}
.metric-card:active { opacity: .85; }
.mc--warning { border-color: var(--warning-border); }
.mc--danger  { border-color: var(--danger-border); box-shadow: 0 2px 12px var(--danger-soft); }

.mc-header { display: flex; justify-content: space-between; align-items: flex-start; margin-bottom: 4px; }
.mc-label  { font-size: 13px; font-weight: 600; color: var(--sub-text); }
.mc-value  { font-size: 28px; font-weight: 700; color: var(--text); line-height: 1; margin-bottom: 10px; }
